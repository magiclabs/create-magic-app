...
const LoginWithEmailOtp = ({token, setToken}: LoginProps) => {
	...
	const handleLogin = async (type: LoginType) => {
		if (
			...
			try {
				...
				switch (type) {
					case LoginType.OTP:
						account = await magic?.auth.loginWithEmailOTP({email})
						break

					case LoginType.LINK:
						account = await magic?.auth.loginWithMagicLink({
							email,
						})
						console.log('link: ' + JSON.stringify(account))
						break
					default:
						break
				}
