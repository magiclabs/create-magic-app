...
const EmailLink = ({ token, setToken }: LoginProps) => {
  ...
  const handleLogin = async () => {
    if (!email.match(/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/)) {
      ...
      try {
        ...
        setEmailError(false);
        const account = await magic?.auth.loginWithMagicLink({
          email,
        });
        if (account) {
  ...
  return (
    <Card>
      ...
      <div className="flex flex-col items-center justify-center">
        ...
        >
          {isLoginInProgress ? <Spinner /> : token.length > 0 ? 'Logout' : 'Login with Magic Link'}
        </button>
      </div>
    </Card>
  );
